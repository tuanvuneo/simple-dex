...
export function TxStatus({ hash }: TxStatusProps) {
  ...
  return (
    <div className="flex flex-col shadow-xs mt-3">
      {renderStatus()}
      <div
        className={`rounded-b-lg border border-t-0 ${borderClass} ${bgClass} px-4 py-2.5 text-xs flex flex-col sm:flex-row justify-between sm:items-center gap-2`}
      >
...
export function QueryStatus({ isPending, error }: QueryStatusProps) {
  ...
  if (error) {
    const message = (error as BaseError | null)?.shortMessage ?? error.message;
    return (
